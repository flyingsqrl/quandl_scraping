...
wk_ending = url_lines[2].strip()[33:45]

wk_ending = parser.parse(wk_ending)

wk_ending = pd.to_datetime(wk_ending)   

# Add relevant lines to a list and clean up        
volume_lines = url_lines[6:8]

volume_lines = [x.strip().split(':') for x in volume_lines]

primal_cutout = url_lines[19:20][0].split()
del primal_cutout[1]
